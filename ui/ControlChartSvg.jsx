/**
 * Classic hand-rolled SVG Shewhart plot. PURE: `series` = buildSeries() output,
 * `points` = series.points each with a stable `id`. Sigma-band colours follow the
 * standard green/amber/red convention; the selection ring uses --qc-accent.
 */
...
export function ControlChartSvg({ series, points, showLine, selected, onSelect }) {
  const { limits } = series;
      ...
      {points.map((p) => {
        ...
        return (
          <g key={p.id} onClick={() => onSelect(p.id)} style={{ cursor: "pointer" }}>
            {isSel && <circle cx={x(p.date)} cy={y(p.value)} r="8" fill="none" stroke="var(--qc-accent, #0b6e4f)" strokeWidth="2" />}
            <rect x={x(p.date) - 3} y={y(p.value) - 3} width="6" height="6" fill={p.isNew ? "var(--qc-accent, #0b6e4f)" : "#111"} stroke={p.isNew ? "#000" : "none"} strokeWidth={p.isNew ? 1.5 : 0}>
