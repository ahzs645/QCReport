/**
 * Presets / reference page — every constant the QCReport calculations use,
 * rendered as one page: the reportable decision tree, hardness, the QC equations
 * and per-role settings, acceptance ranges with their ± tolerances and SOP
 * citations, the conditional QC interpretations, control-chart math and the
 * sample/analyte matching strategies.
 *
 * PURE / presentational: renders from props only, no engine import. The host
 * supplies the constants — the whole bundle is the module namespace of the light
 * "qcreport/presets" subpath, so no engine/spreadsheet code is pulled in:
 *
 *   import * as presets from "qcreport/presets";
 *   <PresetsReference presets={presets} />
 *
 * `criteria` optionally overrides presets.DEFAULT_QC_CRITERIA — pass the object
 * parseQcCriteria() read off a loaded workbook's CODES sheet to show the live
 * acceptance criteria instead of the encoded fallback.
 */

const range = (r) => (Array.isArray(r) ? `${r[0]} – ${r[1]} %` : "—");

export function PresetsReference({ presets, criteria }) {
  ...
  const c = criteria || DEFAULT_QC_CRITERIA;
  if (!c) return null;
  const live = Boolean(criteria); // criteria came from a loaded workbook, not the encoded fallback
  ...
  return (
    <div className="presets">
      <p className="sub">
        ...
        RECOVERIES” table when a workbook is dropped, and the values shown here are{" "}
        {live ? (
          "the ones read from the loaded workbook"
        ) : (
          <>the encoded fallback (editable in <code>scripts/lib/</code>)</>
        )}.
      </p>
